...
void main() {
  ...
  group('AuthBloc Tests', () {
    final dummyUser = UserModel(id: 1, token: "abcd1234");
    ...
    blocTest<AuthBloc, BaseState>(
      ...
      expect: () => [isA<LoadingState>(), isA<AuthSuccessState>()],
      verify: (_) {
        verify(
          () => mockAuthRepository.register("test@email.com", "password"),
        ).called(1);
      },
    );
    ...
    blocTest<AuthBloc, BaseState>(
      ...
      expect: () => [isA<LoadingState>(), isA<AuthFailureState>()],
    );
  });
}
